...
Starting from ckb-std 0.16.0, RISC-V atomic instructions are generated by default. However, ckb-vm does not directly support atomic instructions. To address this, ckb-std provides the following solution:

> Adjust Rust compilation flags by adding `-C passes=lower-atomic` to `RUSTFLAGS`

The `dummy-atomic` and `target-feature=-a` combination is no longer supported after Rust 1.95. `dummy-atomic` will be removed in a future release.

For more details on compilation flags, refer to the [CKB Script Templates repository](https://github.com/cryptape/ckb-script-templates/tree/main?tab=readme-ov-file#molecule-uses-bytes-crates).
